...
from tensorrt_llm.models.hydra.weight import (capture_activation_range,
                                              convert_hf_llama, load_hydra_hf)
...
def parse_arguments():
    ...
    parser.add_argument(
        '--model_dir',
        type=str,
        help="base model or Hydra-enhanced quantized model from ModelOpt")
    ...
    parser.add_argument('--num_hydra_heads', type=int, default=4)
    parser.add_argument('--num_hydra_layers', type=int, default=1)
    parser.add_argument('--max_hydra_token_len', type=int, default=63)
    parser.add_argument('--hydra_hidden_act', type=str, default="silu")
    parser.add_argument('--hydra_model_dir', type=str, default=None)
...
def main():
    ...
    if quant_config_file.exists():
        ...
        if is_modelopt_ckpt:  # quantized ckpt
            args.hydra_model_dir = args.model_dir
            args.num_hydra_heads = hf_config.hydra["num_hydra_heads"]
            args.num_hydra_layers = hf_config.hydra["num_hydra_layers"]
    ...
    config = {
        ...
        'max_draft_len': args.max_hydra_token_len,
        'num_hydra_heads': args.num_hydra_heads,
        'num_hydra_layers': args.num_hydra_layers,
    ...
    def covert_and_save(rank, convert_args):
        ...
        if args.use_weight_only and args.weight_only_precision == 'int4_gptq':
            ...
            if args.load_by_shard:
                ...
                if args.hydra_model_dir is not None:
                    config_file = Path(args.hydra_model_dir) / "config.json"
                    with open(config_file) as fp:
                        config = json.load(fp)
                    if not convert_args["is_modelopt_ckpt"]:
                        num_hydra_heads_from_config = config.get(
                            'hydra_num_heads', args.num_hydra_heads)
                        args.num_hydra_layers = config.get(
                            'hydra_num_layers', args.num_hydra_layers)
                    if args.num_hydra_heads is None:
                        args.num_hydra_heads = num_hydra_heads_from_config

                    assert args.max_hydra_token_len > 0, "should have max_hydra_token_len > 0"

                    hydra_weights = load_hydra_hf(
                        hydra_path=args.hydra_model_dir,
                        num_hydra_heads=args.num_hydra_heads,
                        num_hydra_layers=args.num_hydra_layers,
                        base_config=hf_config,
                        mapping=mapping,
                        dtype=args.dtype,
                        use_weight_only=args.use_weight_only,
                        plugin_weight_only_quant_type=
